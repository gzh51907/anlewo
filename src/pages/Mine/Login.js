...
class Login extends Component {
    ...
    inputPhone = async (ev) => {
        ...
        if (isOK) {
            let { data } = await axios.get('http://47.98.245.185:1998/users/check', {
                params: {
        ...
        if (this.refs.phoneNumber.value && this.refs.verify.value) {
            if (havephone) {
                if (this.state.code == this.refs.verify.value) {
                    if (reg) {
                        let { data: data1 } = await axios.post('http://47.98.245.185:1998/users/reg', {
                            phone
                        });
                        console.log(data1);
                        let { data: { data: data2 } } = await axios.get("http://47.98.245.185:1998/users/login", {
                            params: {
                                phone
                            }
                        });
